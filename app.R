...
server <- function(input, output, session) {
  ...
  observeEvent(input$map_click, {
    ...
    # Fetch images for species
    images <<- lapply(unique(points_in_annulus$species), function(species) {
      image_url <- get_images(species)
    ...
    output$imageGrid <- renderUI({
      image_elements <- lapply(images, function(image_url) {
        if (!is.null(image_url)) {
          tags$img(src = image_url, alt = "Species image")
        } else {
          NULL
        }
      })
